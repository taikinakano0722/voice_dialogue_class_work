...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    cached_features_file = os.path.join(args.data_dir, 'sft_{}_{}_{}_{}'.format( #加工したデータの保存先
        args.data_name,
        mode,
    ...
    if os.path.exists(cached_features_file):
        ...
        features = convert_to_features(args, tokenizer, mode) #source_id,target_idはここで取得される
        print("Loaded number of instance:", len(features['source_ids']))
    ...
    with open(path, 'r', encoding='utf-8') as infile:
        ...
        if args.data_name in ['esc','cb']:
            for line in infile:
                ...
                for turn in dial:
                    if turn['speaker'] == 'sys' and len(state) > 0:
                        ...
                        target_id = act.index(turn['strategy']) #target_idはstrategy
            ...
            for line in infile:
                ...
                target_id = act.index(sample['strategy']) #target_idはstrategy
                dial_id = []
                for s in dial:
