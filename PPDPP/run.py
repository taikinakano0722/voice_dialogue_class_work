...
def train(args, config, dataset, filename, tokenizer):
    ...
    if args.sft_dir is not None: #事前学習済みのモデル (Supervised Fine-Tuning, SFT) がある場合、そのモデルをロード
    ...
    test_performance = []
    if args.do_eval: #args.do_eval が True の場合、初期状態でモデルを評価
    ...
    for train_step in range(1, args.max_steps+1):
        print(f"\n現在のトレーニング回数:{train_step}\n")
        #SR : 成功率 (Success Rate)
        #AvgT : 平均ターン数 (Average Turns)
        #total_reward : 累積報酬
