...
def train(args, train_dataset, model, tokenizer):
    tb_writer = SummaryWriter() #TensorBoardに学習過程を記録するオブジェクトを作成


    args.train_batch_size = args.per_gpu_train_batch_size * max(1, len(args.device_id)) #train_batch_sizeをGPUの数に応じて設定
    train_dataloader = DataLoader(DataFrame(train_dataset, args), batch_size=args.train_batch_size, shuffle=True, collate_fn=collate_fn) #train_dataset を DataLoader に変換し、ミニバッチを作成 できるように

    t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs #総トレーニングステップ数を計算.学習率のスケジューラで使用

    # Prepare optimizer and schedule (linear warmup and decay)
    no_decay = ['bias', 'LayerNorm.weight'] #重み減衰（L2正則化）を適用しないパラメータのリスト（LayerNorm と bias は正則化しない）。
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)],
         'weight_decay': args.weight_decay}, #正則化を適用するもの 
        {'params': [p for n, p in model.named_parameters() if any(nd in n for nd in no_decay)], 'weight_decay': 0.0} #正則化しないもの
    ]
    #オプティマイザ
    ...
    tr_loss, logging_loss = 0.0, 0.0 #累積損失を0にリセット
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch")
    utils.set_random_seed(args.seed)  # ランダムシードを固定
    global_step = 0 #トレーニングの進行ステップを管理する変数

    #total_rouge = evaluate(args, model, tokenizer, save_output=True)
    best_f1 = 0 #最高のF1スコアを記録するための変数
    
    for e in train_iterator:#エポックごとループ
        logging.info("training for epoch {} ...".format(e))
        print("training for epoch {} ...".format(e))
        epoch_iterator = tqdm(train_dataloader, desc="Iteration")
        for step, batch in enumerate(epoch_iterator):#ミニバッチ単位でデータを処理
            model.train() #モデルをトレーニングモードに
            #batch = tuple(t.to(args.device) for t in batch)
            inputs = {'input_ids': batch['input_ids'].to(args.device),
                        'attention_mask': batch['attention_mask'].to(args.device),
                        'labels': batch['labels'].to(args.device)}
            outputs = model(**inputs) #データをモデルに入力
            loss = outputs#[0]  # model outputs are always tuple in pytorch-transformers (see doc)

            if len(args.device_id) > 1:
                loss = loss.mean()  # mean() to average on multi-gpu parallel training
            if args.gradient_accumulation_steps > 1: #勾配を蓄積用に分割
                loss = loss / args.gradient_accumulation_steps

            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm) #勾配クリッピング

            tr_loss += loss.item()
            if (step + 1) % args.gradient_accumulation_steps == 0: #勾配蓄積を考慮して学習
...
def evaluate(args, model, tokenizer, save_output=False):
    eval_dataset = data_reader.load_and_cache_examples(args, tokenizer, evaluate=True) #評価データのロードおよびトークン化
...
def main():
    ...
    ## Required parameters
    parser.add_argument('--data_name', default='esc', type=str, #cb,cima,esc
                        help="dataset name")
    ...
    train_dataset = data_reader.load_and_cache_examples(args, tokenizer, evaluate=False) #ここでデータセット内の文章をトークン化してキャッシュへ保存
